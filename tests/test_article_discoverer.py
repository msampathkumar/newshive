"""
Tests for ArticleDiscoverer — link extraction, delta computation,
...
@pytest.fixture
def discoverer(storage_mock):
    return ArticleDiscoverer(storage=storage_mock)


# ── extract_child_urls ──────────────────────────────────────────────────────

def test_extract_child_urls_returns_absolute_urls(discoverer):
    urls = discoverer.extract_child_urls(SAMPLE_HTML, BASE_URL)
...
def test_extract_child_urls_strips_fragments(discoverer):
    html = '<html><body><a href="/blog/post-1#section">Link</a></body></html>'
    urls = discoverer.extract_child_urls(html, BASE_URL)
    assert "https://example.com/blog/post-1" in urls
    # Fragment version should NOT be present
    assert not any("#" in u for u in urls)


def test_extract_child_urls_empty_html(discoverer):
    urls = discoverer.extract_child_urls("<html><body></body></html>", BASE_URL)
    assert urls == set()


# ── compute_delta ───────────────────────────────────────────────────────────

def test_compute_delta_returns_new_only(discoverer):
    ...
    delta   = discoverer.compute_delta(current, prior)
    assert delta == {"https://example.com/blog/3"}


def test_compute_delta_empty_prior(discoverer):
    current = {"https://example.com/blog/1"}
    delta = discoverer.compute_delta(current, set())
    assert delta == current


def test_compute_delta_no_new_urls(discoverer):
    current = {"https://example.com/blog/1"}
    delta = discoverer.compute_delta(current, current)
    assert delta == set()


# ── filter_same_domain ───────────────────────────────────────────────────────

def test_filter_same_domain_keeps_sub_path(discoverer):
    urls = {
        "https://example.com/blog/post-1",
        "https://example.com/blog/nested/post",
    }
    result = discoverer.filter_same_domain("https://example.com/blog", urls)
    assert "https://example.com/blog/post-1" in result
    assert "https://example.com/blog/nested/post" in result


def test_filter_same_domain_drops_different_path(discoverer):
    urls = {
...
def test_filter_same_domain_drops_external(discoverer):
    urls = {"https://other.com/blog/post"}
    result = discoverer.filter_same_domain("https://example.com/blog", urls)
    assert len(result) == 0


def test_filter_same_domain_excludes_index_itself(discoverer):
...
def test_get_prior_links_falls_back_to_older_folder(storage_mock, discoverer):
    ...
    prior = discoverer.get_prior_links("https://example.com/blog")
...
def test_get_prior_links_returns_empty_when_no_prior(storage_mock, discoverer):
